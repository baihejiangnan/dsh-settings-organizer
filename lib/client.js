  ...
  factory: function (require) {
    ...
    function syncVisibility() {
      document.querySelectorAll(".dsho-nav-head").forEach(function (head) {
        ...
        if (!group) return;
        head.hidden = group.hidden || ancestorHidden(group);
        head.textContent = group.label;
      });
      managed.forEach(function (_, row) {
        var group = groupById(row.getAttribute("data-dsho-group"));
        row.hidden = !group || group.hidden || ancestorHidden(group);
      });
    }
    ...
    function navCss() {
      var s = document.createElement("style");
      s.textContent =
        ".dsho-organized{overflow-y:auto;scrollbar-gutter:stable}.dsho-nav-head{width:100%;box-sizing:border-box;padding:9px 16px;color:#697386;font-size:13px;font-weight:600}.dsho-managed-row{padding-left:28px!important}.dsho-nav-head[hidden],.dsho-managed-row[hidden]{display:none!important}";
      document.head.appendChild(s);
    }
    function move(id, d) {
      var a = copy(state);
