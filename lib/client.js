  ...
  factory: function (require) {
    ...
    function organize(force) {
      ...
      timer = setTimeout(function () {
        if (!force && document.querySelector(".dsho-nav-group")) return;
        restore();
        ...
        state.nodes.forEach(function (g) {
          ...
          body.className =
            "dsho-nav-items" + (state.collapsed[g.id] ? " is-collapsed" : "");
          inner.className = "dsho-nav-items-inner";
          h.onclick = function () {
            var next = copy(state);
            var collapsed = !body.classList.contains("is-collapsed");
            next.collapsed[g.id] = collapsed;
            body.classList.toggle("is-collapsed", collapsed);
            h.setAttribute("aria-expanded", collapsed ? "false" : "true");
            h.textContent = (collapsed ? "▸ " : "▾ ") + g.label;
            persist(next);
          };
          body.appendChild(inner);
          box.append(h, body);
          containers[g.id] = { box: box, body: body, inner: inner };
        ...
        state.nodes.forEach(function (g) {
          ...
          (parent ? parent.inner : n).appendChild(current.box);
        });
      }, force ? 0 : 50);
    }
    function css() {
    ...
    function navCss() {
      var s = document.createElement("style");
      s.textContent =
        ".dsho-organized{overflow-y:auto;scrollbar-gutter:stable}.dsho-nav-group{width:100%}.dsho-nav-head{width:100%;border:0;background:transparent;text-align:left;padding:9px 16px;color:#697386;font-size:13px;font-weight:600;cursor:pointer}.dsho-nav-items{display:grid;grid-template-rows:1fr;opacity:1}.dsho-nav-items-inner{min-height:0;overflow:hidden;display:flex;flex-direction:column}.dsho-nav-items.is-collapsed{grid-template-rows:0fr;opacity:.65}.dsho-nav-items-inner>button,.dsho-nav-items-inner>a{padding-left:28px!important}@media(prefers-reduced-motion:no-preference){.dsho-nav-items{transition:grid-template-rows .18s ease,opacity .14s ease}.dsho-nav-head{transition:background-color .14s ease,color .14s ease}.dsho-nav-head:hover{background:rgba(37,99,235,.06)}}";
      document.head.appendChild(s);
    }
    function move(id, d) {
      var a = copy(state);
