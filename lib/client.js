window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    var module = { exports: {} },
      ...
      REMOTE = "/settings-organizer/api/config";
    var DEFAULT = {
      version: 2,
      showEmpty: false,
      safeMode: false,
    ...
    function readJsonResponse(response) {
      return response.text().then(function (text) {
        var body = text.trim() ? JSON.parse(text) : {};
        if (!response.ok)
          throw new Error(body.error || "request failed (" + response.status + ")");
        return body;
      });
    }
    function saveRemote(x) {
      return fetch(REMOTE, {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ config: x }),
      }).then(readJsonResponse).catch(function (error) {
        console.warn("[dsh-settings-organizer] profile save failed:", error);
      });
    }
    function hydrateRemote() {
      return fetch(REMOTE, { headers: { accept: "application/json" } })
        .then(readJsonResponse)
        .then(function (body) {
          if (body.config && body.config.version === 2 && Array.isArray(body.config.nodes)) {
            state = normalizeConfig(body.config);
        ...
        .catch(function (error) {
          console.warn("[dsh-settings-organizer] profile load failed:", error);
        });
    }
    function persist(x) {
      state = x;
