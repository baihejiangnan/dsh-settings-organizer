window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function restore() {
      moved.forEach(function (v, row) {
        if (!v.parent || !v.parent.isConnected) return;
        var reference =
          v.next && v.next.parentNode === v.parent ? v.next : null;
        v.parent.insertBefore(row, reference);
      });
      moved.clear();
      document.querySelectorAll(".dsho-nav-group").forEach(function (x) {
        x.remove();
      });
    }
    function organize() {
      ...
      timer = setTimeout(function () {
        if (document.querySelector(".dsho-nav-group")) return;
        restore();
        ...
        state.nodes.forEach(function (g) {
          ...
          box.className = "dsho-nav-group";
          h.className = "dsho-nav-head";
          h.style.paddingLeft = 16 + (groupDepth(g) - 1) * 16 + "px";
          h.type = "button";
          h.setAttribute("aria-expanded", state.collapsed[g.id] ? "false" : "true");
          h.textContent = (state.collapsed[g.id] ? "▸ " : "▾ ") + g.label;
          body.className = "dsho-nav-items";
          body.hidden = state.collapsed[g.id] === true;
          h.onclick = function () {
            var next = copy(state);
            next.collapsed[g.id] = body.hidden === false;
            save(next);
          };
          box.append(h, body);
          containers[g.id] = { box: box, body: body };
          list.forEach(function (row) {
            moved.set(row, { parent: n, next: row.nextSibling });
            body.appendChild(row);
          });
        });
        state.nodes.forEach(function (g) {
          var current = containers[g.id];
          if (!current) return;
          var parent = g.parentId && containers[g.parentId];
          (parent ? parent.body : n).appendChild(current.box);
        });
      }, 50);
    }
    function css() {
      ...
      s.id = "dsho-css";
      s.textContent =
        ".dsho-card{max-width:760px;padding:20px}.dsho-tools{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}.dsho-btn{border:1px solid var(--ds-border,#d9dde5);background:var(--ds-bg,#fff);border-radius:7px;padding:7px 11px;cursor:pointer}.dsho-primary{background:#2563eb;color:#fff;border-color:#2563eb}.dsho-tree{border:1px solid var(--ds-border,#e3e7ee);border-radius:9px;overflow:hidden}.dsho-row{display:flex;align-items:center;gap:8px;padding:9px 11px;border-bottom:1px solid var(--ds-border,#edf0f4)}.dsho-group{background:var(--ds-bg-secondary,#f7f8fa);font-weight:600}.dsho-child{padding-left:32px}.dsho-label{flex:1}.dsho-actions{display:flex;gap:2px}.dsho-actions button{border:0;background:transparent;padding:4px 6px;cursor:pointer;color:var(--ds-text-secondary,#697386)}.dsho-hint{font-size:13px;color:var(--ds-text-secondary,#697386);line-height:1.5}@media(prefers-reduced-motion:no-preference){.dsho-row{transition:background .14s ease}.dsho-row:hover{background:rgba(37,99,235,.06)}}";
      document.head.appendChild(s);
    }
    function navCss() {
      var s = document.createElement("style");
      s.textContent =
        ".dsho-organized{overflow-y:auto;scrollbar-gutter:stable}.dsho-nav-group{width:100%}.dsho-nav-head{width:100%;border:0;background:transparent;text-align:left;padding:9px 16px;color:#697386;font-size:13px;font-weight:600;cursor:pointer}.dsho-nav-items{display:flex;flex-direction:column}.dsho-nav-items[hidden]{display:none!important}.dsho-nav-items>button,.dsho-nav-items>a{padding-left:28px!important}";
      document.head.appendChild(s);
    }
    function move(id, d) {
      var a = copy(state);
        ...
        var result = n
          ? Array.from(n.querySelectorAll("button,a"))
              ...
              .filter(function (x, i, a) {
                return x && core.indexOf(x) < 0 && a.indexOf(x) === i;
              })
          : [];
        return result;
      }
      ...
      function add() {
        ...
        var parent = prompt(
          "父分组 ID（留空表示一级）\n" +
            state.nodes
              .map(function (n) {
                return n.id + " = " + groupPath(n);
              })
              .join("\n"),
          "",
        );
        parent = parent && groupById(parent) ? parent : null;
        if (parent && groupDepth(groupById(parent)) >= 3) {
          alert("最多支持三级导航");
          return;
        }
        var x = copy(state);
