  ...
  factory: function (require) {
    ...
    var DEFAULT = {
      ...
      nodes: [
        ...
        { id: "automation", label: "Agent 与自动化", children: [] },
        { id: "developer", label: "开发工具", children: [] },
        { id: "external", label: "外部服务", children: [] },
        { id: "other", label: "其他扩展", children: [] },
      ],
    };
    function copy(x) {
    ...
    function load() {
      try {
        ...
        return x && x.version === 2 ? x : copy(DEFAULT);
      } catch (_) {
        return copy(DEFAULT);
      }
    }
    ...
    function organize() {
      ...
      timer = setTimeout(function () {
        ...
        state.nodes.forEach(function (g) {
          var list = b[g.id] || [];
          if (!list.length && !state.showEmpty) return;
    ...
    function Editor2() {
      ...
      function add() {
        ...
        if (!l) return;
        var x = copy(state);
        x.nodes.push({ id: "group-" + Date.now(), label: l, children: [] });
        save(x);
        redraw();
      }
      function reset() {
