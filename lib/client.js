window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function restore() {
      ...
      managed.clear();
      document.querySelectorAll(".dsho-nav-head").forEach(function (x) {
        x.remove();
      });
    }
    function ancestorCollapsed(group) {
      var current = group;
      while (current && current.parentId) {
        current = groupById(current.parentId);
    ...
    function syncCollapsed() {
      document.querySelectorAll(".dsho-nav-head").forEach(function (head) {
        var group = groupById(head.getAttribute("data-dsho-group"));
        if (!group) return;
        var collapsed = state.collapsed[group.id] === true;
        head.hidden = group.hidden || ancestorCollapsed(group);
      ...
      managed.forEach(function (_, row) {
        ...
        row.hidden = !group || group.hidden || state.collapsed[group.id] === true || ancestorCollapsed(group);
      });
    }
    function organize(force) {
      clearTimeout(timer);
      timer = setTimeout(function () {
        if (!force && document.querySelector(".dsho-nav-head")) return;
        restore();
        ...
        state.nodes.forEach(function (g) {
          ...
          h.setAttribute("data-dsho-group", g.id);
    ...
    function navCss() {
      var s = document.createElement("style");
      s.textContent =
        ".dsho-organized{overflow-y:auto;scrollbar-gutter:stable}.dsho-nav-head{width:100%;border:0;background:transparent;text-align:left;padding:9px 16px;color:#697386;font-size:13px;font-weight:600;cursor:pointer}.dsho-managed-row{padding-left:28px!important}.dsho-nav-head[hidden],.dsho-managed-row[hidden]{display:none!important}@media(prefers-reduced-motion:no-preference){.dsho-nav-head,.dsho-managed-row{transition:opacity .14s ease,background-color .14s ease,color .14s ease}.dsho-nav-head:hover{background:rgba(37,99,235,.06)}}";
      document.head.appendChild(s);
    }
    function move(id, d) {
      var a = copy(state);
    ...
    function Editor2() {
      var rev = React.useState(0),
        ...
        parentState = React.useState(""),
        newParent = parentState[0],
        setNewParent = parentState[1];
      function redraw() {
        setRev(function (x) {
          return x + 1;
        });
      ...
      function add() {
        var l = newName.trim();
        if (!l) return;
        var parent = newParent && groupById(newParent) ? newParent : null;
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            "select",
            {
              className: "dsho-select dsho-parent-select",
              value: newParent,
              onChange: function (e) { setNewParent(e.target.value); },
            },
            React.createElement("option", { value: "" }, "一级分组"),
            state.nodes.filter(function (group) {
              return groupDepth(group) < 3;
            }).map(function (group) {
              return React.createElement(
                "option",
